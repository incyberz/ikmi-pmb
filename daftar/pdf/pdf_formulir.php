...
if($id_jalur==3) $id_persyaratan = 3; //bukti kip
...
$q = mysqli_query($cn,$s) or die(mysqli_error($cn));
if(mysqli_num_rows($q)!=1) die("Persyaratan tidak ditemukan. ".mysqli_error($cn));
while ($d=mysqli_fetch_assoc($q)) {
...
if(strlen($no_ayah)>9) $no_ayah_show = substr($no_ayah, 0,3)."***".substr($no_ayah, 9,4);
if(strlen($no_ibu)>9) $no_ibu_show = substr($no_ibu, 0,3)."***".substr($no_ibu, 9,4);
if(strlen($no_saudara)>9) $no_saudara_show = substr($no_saudara, 0,3)."***".substr($no_saudara, 9,4);

if($no_ayah=="") $no_ayah_show = "-";
if($no_ibu=="") $no_ibu_show = "-";
...
if(!file_exists($foto_profil)) die("Foto Profil not found.<hr><small><i>foto_profil: $foto_profil</i></small>");

// $pdf->Cell( 30, 30, $pdf->Image($foto_profil, $pdf->GetX(), $pdf->GetY(), 25), 0, 0, 'L', false );
$pdf->Ln(5);
...
$pdf->Ln(3);
$pdf->Cell(190,10,"$nama_persyaratan telah terverifikasi pada $tanggal_verifikasi_upload oleh Petugas PMB STMIK IKMI Cirebon","TB",0);
$pdf->Ln(5);
