...
if(mysqli_num_rows($q)!=1) die("id_jadwal_tes tidak ditemukan pada id_daftar: $id_daftar");
$d = mysqli_fetch_assoc($q);
$id_jadwal_tes = $d['id_jadwal_tes'];
$titi_mangsa = $d['titi_mangsa'];
$tanggal_deadline = $d['tanggal_deadline'];

if($titi_mangsa=="")die("Tanggal Surat belum ditentukan. 
...
if($tanggal_deadline=="")die("Tanggal Deadline belum ditentukan. 
    Silahkan hubungi Petugas PMB.
    <hr>
    id_daftar:$id_daftar<br>
    id_jadwal_tes:$id_jadwal_tes<br>
...
if($grade_lulus=="" and $id_jalur!=3) die("grade_lulus is empty.<hr>Fitur ini tidak bisa diakses tanpa Grade Lulus yang benar.");
if(strlen($grade_lulus)>1) die("grade_lulus length invalid.");
...
# ===========================================
if(!isset($_POST['download_formulir_token'])) die("Page ini idak dapat diakses secara langsung. <hr>Download Token is missing.");

$download_formulir_token = $_POST['download_formulir_token'];
$token2 = md5(date("mydh").$id_jalur);
if($download_formulir_token!=$token2) die("Token invalid untuk mengakses fitur ini.");
...
$pdf->Cell(23,1,' ',0,1);
// nomor
$singkatan_jalur_surat = "REG"; if($id_jalur==3) $singkatan_jalur_surat = "KIP";
...
$pdf->Cell(145,5,"Pemberitahuan Hasil Seleksi Calon Mahasiswa Baru TA. 2022/2023",$cb,1);
$pdf->SetFont('Arial','',9);
...
if($id_jalur==3){
    $pdf->Cell(170,5,"Kami ucapkan selamat kepada saudara/i yang telah dinyatakan LULUS dan DITERIMA seleksi Tes dan",$cb,1);
    $pdf->Cell(170,5,"Wawancara Untuk Diproses Lebih Lanjut sebagai Calon Mahasiswa Penerima Beasiswa KIP Kuliah",$cb,1);
    $pdf->Cell(170,5,"STMIK IKMI Cirebon Tahun Akademik $tahun_akademik pada :",$cb,1);
}else{
    // jalur REGULER
    $pdf->Cell(170,5,"Kami ucapkan selamat kepada saudara yang telah dinyatakan LULUS GRADE $grade_lulus sebagai Calon Mahasiswa pada jalur ",$cb,1);
    $pdf->Cell(170,5,"Reguler STMIK IKMI Cirebon dalam seleksi PMB Tahun Akademik 2022/2023, pada:",$cb,1);
}
...
if($id_jalur==3){
    ...
    $pdf->Cell(170,2,' ',"",1);
    // dengan hormat
    $pdf->Cell(170,5,"Selanjutnya saudara dipersilakan untuk melakukan Daftar Ulang (Registrasi Ulang) dengan cara ",$cb,1);
    $pdf->Cell(170,5,"mengirimkan Dokumen Persyaratan sebagai berikut:",$cb,1);
    ...
    $pdf->Cell(165,1," ",$cb,1);
    $pdf->SetFont('Arial','',8);
    $pdf->Cell(165,4,"1. Bukti Pendaftaran KIP Kuliah 2022 (Contoh dapat dilihat pada lampiran). Apabila belum melakukan",$cb,1);
    $pdf->Cell(165,4,"    pendaftaran Akun KIP Kuliah Kemdikbud bisa kami bantu proses pendaftarannya dengan cara chat",$cb,1);
    $pdf->Cell(165,4,"    melalui Whatsapp di Nomor 0838-2165-1265 atau 0821-3014-8448.",$cb,1);
    $pdf->Cell(165,4,"2. Surat Pernyataan yang harus di download melalui link https://bit.ly/pernyataan-kipk-2022",$cb,1);
    $pdf->Cell(165,4,"3. Pas Foto Ukuran 4 x 6 background biru (2 lembar)",$cb,1);
    $pdf->Cell(165,4,"4. Fotocopy KTP Calon Mahasiswa / Surat Keterangan Disdukcapil (2 lembar)",$cb,1);
    $pdf->Cell(165,4,"5. Fotocopy KTP Orang Tua (2 lembar)",$cb,1);
    $pdf->Cell(165,4,"6. Fotocopy Ijazah Terakhir / Surat Keterangan Sebagai Siswa Kelas 12 (1 lembar)",$cb,1);
    $pdf->Cell(165,4,"7. Fotocopy Fotocopy Kartu Keluarga (2 lembar)",$cb,1);
    $pdf->Cell(165,4,"8. Fotocopy KIP Pelajar (jika ada) / KKS / PKH / SKTM dari desa / kelurahan (1 lembar)",$cb,1);
    $pdf->Cell(165,4,"9. Foto Bersama Keluarga termasuk Calon Mhs didalamnya dicetak kertas foto ukuran 4R (1 lembar)",$cb,1);
    $pdf->Cell(165,4,"10. Foto Tampak Depan Rumah dicetak kertas foto ukuran 4 R. (1 lembar)",$cb,1);
    $pdf->Cell(165,4,"11. Foto Ruang Keluarga dicetak kertas foto ukuran 4 R. (1 lembar)",$cb,1);
    $pdf->SetMargins(20,10,20);
    $pdf->SetFont('Arial','',9);
    ...
    # ===========================================
    /* Selanjutnya saudara dipersilakan untuk melakukan Daftar Ulang (Registrasi) dengan cara menyelesaikan pembayaran kewajiban melalui Transfer atau M-Banking BANK BRI  Nomor Rekening 4149-01-000004-30-5 an STMIK IKMI Cirebon dan melakukan konfirmasi ke Bagian Administrasi Umum (BAU) di nomor 0823-1111-3070. Apabila melewati dari tanggal yang telah ditentukan, maka kami anggap anda telah mengundurkan diri dari Calon Mahasiswa STMIK IKMI Cirebon. */
    # ===========================================
    $pdf->Cell(170,5,"Selanjutnya saudara dipersilakan untuk melakukan Daftar Ulang (Registrasi) dengan cara menyelesaikan pembayaran ",$cb,1);
    $pdf->Cell(170,5,"kewajiban melalui Transfer atau M-Banking BANK BRI  Nomor Rekening 4149-01-000004-30-5 an STMIK IKMI Cirebon ",$cb,1);
    $pdf->Cell(170,5,"dan melakukan konfirmasi ke Bagian Administrasi Umum (BAU) di nomor 0823-1111-3070. Apabila melewati dari ",$cb,1);
    $pdf->Cell(170,5,"tanggal yang telah ditentukan, maka kami anggap anda telah mengundurkan diri dari Calon Mahasiswa STMIK  ",$cb,1);
    $pdf->Cell(170,5,"IKMI Cirebon. ",$cb,1);
    $pdf->Cell(170,2,"  ",$cb,1);
    ...
    # ===========================================
    $pdf->Cell(170,5,"Untuk selanjutnya menyerahkan kelengkapan dokumen persyaratan di Sekretariat Pendaftaran STMIK IKMI Cirebon ",$cb,1);
    $pdf->Cell(170,5,"pada hari kerja Senin sd Sabtu (Pukul 08.00 sd 16.00 WIB) sebagai berikut :",$cb,1);
    $pdf->Cell(170,2," ",$cb,1);
    ...
    */
    # ===========================================
    $pdf->Cell(170,5,"1. Bukti Pembayaran (biaya daftar, biaya awal, dan biaya kuliah semester 1) lihat di tabel kewajiban pembayaran.",$cb,1);
    $pdf->Cell(170,5,"2. Pas Foto Ukuran 3x4 dan 4 x 6 background biru (masing-masing 2 lembar).",$cb,1);
    $pdf->Cell(170,5,"3. Fotocopy KTP Calon Mahasiswa / Surat Keterangan Disdukcapil (2 lembar).",$cb,1);
    $pdf->Cell(170,5,"4. Fotocopy KTP Orang Tua",$cb,1);
    $pdf->Cell(170,5,"5. Fotocopy Ijazah Terakhir / Surat Keterangan Sebagai Siswa Kelas 12 (1 lembar).",$cb,1);
    $pdf->Cell(170,5,"6. Fotocopy SKHUN (jika ada).",$cb,1);
    $pdf->Cell(170,5,"7. Fotocopy Fotocopy Kartu Keluarga (2 lembar).",$cb,1);
    $pdf->Cell(170,2," ",$cb,1);
    ...
    */
    # ===========================================
    $pdf->Cell(170,5,"Tabel Kewajiban Pembayaran :",$cb,1);
    $pdf->Cell(170,2," ",$cb,1);

    $pdf->SetFont('Arial','B',9);
    $pdf->Cell(170,5,"A. Biaya Pendaftaran",$cb,1);
    $pdf->SetFont('Arial','',9);

    $pdf->Cell(170,5,"     - Pendaftaran  Rp. 200.000 (Dua ratus ribu rupiah)",$cb,1);

    $pdf->SetTextColor(255,0,0);
    $pdf->Cell(170,5,"     - dilunasi sebelum tanggal $tanggal_deadline",$cb,1);
    $pdf->SetTextColor(0,0,0);
    $pdf->Cell(170,2," ",$cb,1);
    ...
    */
    ...
    $pdf->Cell(70,5,"Jaket Almamater",1,0);
    $pdf->Cell(30,5,"200.000",1,0,"R");
    $pdf->Cell(30,5,"200.000",1,1,"R");

    // 2   PKKMB/KU    250.000 250.000
    $pdf->Cell(6,5," ",0,0);
    $pdf->Cell(7,5,"2",1,0,"C");
    $pdf->Cell(70,5,"PKKMB/KU ",1,0);
    $pdf->Cell(30,5,"250.000",1,0,"R");
    $pdf->Cell(30,5,"250.000",1,1,"R");

    // 3 Perpustakaan + Asuransi 400.000 400.000
    $pdf->Cell(6,5," ",0,0);
    $pdf->Cell(7,5,"3",1,0,"C");
    $pdf->Cell(70,5,"Perpustakaan + Asuransi",1,0);
    $pdf->Cell(30,5,"400.000",1,0,"R");
    $pdf->Cell(30,5,"400.000",1,1,"R");

    // 4 KTM (Kartu Tanda Mahasiswa) 100.000 100.000
    $pdf->Cell(6,5," ",0,0);
    $pdf->Cell(7,5,"4",1,0,"C");
    $pdf->Cell(70,5,"KTM (Kartu Tanda Mahasiswa)",1,0);
    $pdf->Cell(30,5,"100.000",1,0,"R");
    $pdf->Cell(30,5,"100.000",1,1,"R");

    // 5 Kemahasiswaan   200.000 200.000
    $pdf->Cell(6,5," ",0,0);
    $pdf->Cell(7,5,"5",1,0,"C");
    $pdf->Cell(70,5,"Kemahasiswaan",1,0);
    $pdf->Cell(30,5,"200.000",1,0,"R");
    $pdf->Cell(30,5,"200.000",1,1,"R");
    ...
    $pdf->SetFont('Arial','B',9);
    $pdf->Cell(6,5," ",0,0);
    $pdf->Cell(77,5,"TOTAL",1,0,"C");
    $pdf->Cell(30,5,"1.150.000",1,0,"R");
    $pdf->Cell(30,5,"1.150.000",1,1,"R");

    // )* Wajib Di Lunasi Sebelum Tanggal $tanggal_deadline
    $pdf->Cell(6,5," ",0,0);
    $pdf->SetTextColor(255,0,0);
    $pdf->Cell(164,5,")* Wajib Di Lunasi Sebelum Tanggal $tanggal_deadline",0,1);
    $pdf->SetTextColor(0,0,0);
    ...
    * pembayaran dilakukan sebelum tanggal $tanggal_deadline 
    ...
    2   Rekayasa Perangkat Lunak (S1)   2.700.000   2.950.000   3.400.000   3.650.000   4.200.000   4.400.000
    3   Sistem Informasi (S1)           2.700.000   2.950.000   3.400.000   3.650.000   4.200.000   4.400.000
    4   Manajemen Informatika (D3)      2.450.000   2.700.000   3.000.000   3.250.000   4.200.000   4.400.000
    5   Komputerisasi Akuntansi (D3)    2.450.000   2.700.000   3.000.000   3.250.000   4.200.000   4.400.000
    ...
    */
    ...
    // No  Program Studi                   GRADE A *)              GRADE B *)              GRADE C *)
    $pdf->Cell(7,6," ",$cb,0,"C");
    $pdf->Cell(55,6," ",$cb,0,"C");
    $pdf->Cell(36,6,"Grade A",$cb,0,"C");
    $pdf->Cell(36,6,"Grade B",$cb,0,"C");
    $pdf->Cell(36,6,"Grade C",$cb,1,"C");

    $pdf->Cell(7,6,"No",$cb,0,"C");
    $pdf->Cell(55,6,"Program Studi",$cb,0,"C");
    $pdf->Cell(18,6,"Pagi",$cb,0,"C");
    $pdf->Cell(18,6,"Sore",$cb,0,"C");
    $pdf->Cell(18,6,"Pagi",$cb,0,"C");
    $pdf->Cell(18,6,"Sore",$cb,0,"C");
    $pdf->Cell(18,6,"Pagi",$cb,0,"C");
    $pdf->Cell(18,6,"Sore",$cb,1,"C");
    ...
    // 1   Teknik Informatika (S1)         3.200.000   3.450.000   3.800.000   4.050.000   4.200.000   4.400.000
    $pdf->Cell(7,5,"1",$cb,0,"C");
    $pdf->Cell(55,5,"Teknik Informatika (S1)",$cb,0);
    $pdf->Cell(18,5,"3.200.000",$cb,0,"R");
    $pdf->Cell(18,5,"3.450.000",$cb,0,"R");
    $pdf->Cell(18,5,"3.800.000",$cb,0,"R");
    $pdf->Cell(18,5,"4.050.000",$cb,0,"R");
    $pdf->Cell(18,5,"4.200.000",$cb,0,"R");
    $pdf->Cell(18,5,"4.400.000",$cb,1,"R");

    // 2   Rekayasa Perangkat Lunak (S1)   2.700.000   2.950.000   3.400.000   3.650.000   4.200.000   4.400.000
    $pdf->Cell(7,5,"2",$cb,0,"C");
    $pdf->Cell(55,5,"Rekayasa Perangkat Lunak (S1)",$cb,0);
    $pdf->Cell(18,5,"2.700.000",$cb,0,"R");
    $pdf->Cell(18,5,"2.950.000",$cb,0,"R");
    $pdf->Cell(18,5,"3.400.000",$cb,0,"R");
    $pdf->Cell(18,5,"3.650.000",$cb,0,"R");
    $pdf->Cell(18,5,"4.200.000",$cb,0,"R");
    $pdf->Cell(18,5,"4.400.000",$cb,1,"R");

    // 3 Sistem Informasi (S1)           2.700.000   2.950.000   3.400.000   3.650.000   4.200.000   4.400.000
    $pdf->Cell(7,5,"3",$cb,0,"C");
    $pdf->Cell(55,5,"Sistem Informasi (S1)",$cb,0);
    $pdf->Cell(18,5,"2.700.000",$cb,0,"R");
    $pdf->Cell(18,5,"2.950.000",$cb,0,"R");
    $pdf->Cell(18,5,"3.400.000",$cb,0,"R");
    $pdf->Cell(18,5,"3.650.000",$cb,0,"R");
    ...
    // 4 Manajemen Informatika (D3)      2.450.000   2.700.000   3.000.000   3.250.000   4.200.000   4.400.000
    $pdf->Cell(7,5,"4",$cb,0,"C");
    $pdf->Cell(55,5,"Manajemen Informatika (D3)",$cb,0);
    $pdf->Cell(18,5,"2.450.000",$cb,0,"R");
    $pdf->Cell(18,5,"2.700.000",$cb,0,"R");
    $pdf->Cell(18,5,"3.000.000",$cb,0,"R");
    $pdf->Cell(18,5,"3.250.000",$cb,0,"R");
    $pdf->Cell(18,5,"4.200.000",$cb,0,"R");
    $pdf->Cell(18,5,"4.400.000",$cb,1,"R");

    // 5 Komputerisasi Akuntansi (D3)    2.450.000   2.700.000   3.000.000   3.250.000   4.200.000   4.400.000
    $pdf->Cell(7,5,"5",$cb,0,"C");
    $pdf->Cell(55,5,"Komputerisasi Akuntansi (D3)",$cb,0);
    $pdf->Cell(18,5,"2.450.000",$cb,0,"R");
    $pdf->Cell(18,5,"2.700.000",$cb,0,"R");
    $pdf->Cell(18,5,"3.000.000",$cb,0,"R");
    $pdf->Cell(18,5,"3.250.000",$cb,0,"R");
    ...
    */
    # ===========================================
    $cb=0;
    $pdf->Cell(170,5,"Informasi teknis Pembayaran bisa menghubungi Chat Whatsapp di nomor :",$cb,1);
    $pdf->Cell(170,5," - 0838-2165-1265 / 0823-1605-5422 ( Front Office )",$cb,1);
    $pdf->Cell(170,5," - 0823-1111-3070 ( Biro Administrasi Umum )",$cb,1);
    $pdf->Cell(170,5," ",$cb,1);
    $pdf->Cell(170,5,"Demikian surat ini kami sampaikan, atas perhatian dan kerjasamanya yang baik kami ucapkan terima kasih.",$cb,1);
    $pdf->Cell(170,5," ",$cb,1);
